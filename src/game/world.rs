use crate::{
    ...
    obj::{
        ...
        pickup::Pickup,
        player::Player,
        weapon::{WeaponDrop, WeaponInstance, WEAPONS},
        Object,
    },
    util::{Point2, Vector2},
};
use ggez::{error::GameError, graphics, Context, GameResult};
...
impl Level {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> GameResult<Self> {
        ...
        loop {
            ...
            match &*buf.trim_end() {
                "" => continue,
                "GRD" => {
                    ret.grid = bincode::deserialize_from(&mut reader)
                ...
                "ENEMIES" => {
                    ret.enemies = bincode::deserialize_from(&mut reader)
                        .map_err(|e| GameError::UnknownError(format!("{:?}", e)))?
                }
                "POINT GOAL" => {
                    ret.exit = Some(
                ...
                "INTELS" => {
                    ret.intels = bincode::deserialize_from(&mut reader)
                        .map(|l: Vec<(f32, f32)>| {
                            l.into_iter().map(|(x, y)| Point2::new(x, y)).collect()
                        })
                        .map_err(|e| GameError::UnknownError(format!("{:?}", e)))?
                }
                "DECORATIONS" => {
                    ret.decorations = bincode::deserialize_from(&mut reader)
                        .map_err(|e| GameError::UnknownError(format!("{:?}", e)))?
                }
                "PICKUPS" => {
                    ret.pickups = bincode::deserialize_from(&mut reader)
                        .map(|l: Vec<((f32, f32), u8)>| {
                            l.into_iter()
                                .map(|((x, y), i)| (Point2::new(x, y), i))
                                .collect()
                        })
                        .map_err(|e| GameError::UnknownError(format!("{:?}", e)))?
                }
                "WEAPONS" => {
                    ret.weapons = bincode::deserialize_from(&mut reader)
                        .map(|l: Vec<((f32, f32), u8)>| {
                            l.into_iter()
                                .map(|((x, y), i)| WEAPONS[i as usize].make_drop(Point2::new(x, y)))
                                .collect()
                        })
                        .map_err(|e| GameError::UnknownError(format!("{:?}", e)))?
                }
    ...
    pub fn save<P: AsRef<Path>>(&self, path: P) -> GameResult<()> {
        ...
        if !self.weapons.is_empty() {
            writeln!(file, "\nWEAPONS")?;
            let pickups: Vec<((f32, f32), u8)> = self
                .weapons
                .iter()
                .map(|w| {
                    ((w.pos.x, w.pos.y), {
...
impl Grid {
    ...
    pub fn ray_cast(&self, from: Point2, dist: Vector2, finite: bool) -> RayCast {
        ...
        loop {
            ...
            let nearest_corner =
                Point2::new(x_dir.on(f32::from(gx) * 32.), y_dir.on(f32::from(gy) * 32.));
